<?php
class Lklm extends CI_Model{
 public function __construct(){
		parent::__construct();
		 $this->form="lkl";//当前操作的数据库表
  ...
  function add($data){
	...
	if(isset($data['position'])){
	 ...
	 //自动提取摘要
		...
		//处理多个图片，默认将字段命名为duo_img 如果用其它字段，则下面代码需要修改
	 if(isset($data['duo_img'])){	
	    foreach($data['duo_img'] as $k => $v){
	      if(empty($v)){unset($data['duo_img'][$k]);}
	    }
	   $data['duo_img']=array2string($data['duo_img']);
	 }
	//删除编辑器参数
...
$.ajax({
    ...
    success: function(data){
		...
		<input id='{$v['field']}' type='hidden' value='{$up_img2}' name='info[{$v['field']}]'>".' <input type="button" class="caij"  value="裁剪图片" onclick="file_corp('."'{$v['field']}'".');this.blur();" />'.$v['tips'];
              break;
			    case "duo_img"://多图展示
			     $model_field[$key]['form']="<div class='duo_img'><div class='ad_img'><a class='bnt_duo bnt_duo2' onclick='ad_img()' href='javascript:void(0)'><img src='".base_url()."views/skin/images/increase.png' /> 增加图片</a></div> <div class='insertimg' id='insertimg'>";
				 //获取图片地址
				 if(isset($data_res[$v['field']])){
				 ...
				 $duo_img=string2array($data_res[$v['field']]);
				  ...
				  foreach($duo_img as $vv){
						...
						if(substr($up_img,0,4)=='http' || substr($up_img,0,1)=='/'){
						   ...
						   $str.="<div class='list' id='list_".$x."'><div class='img'>
        ...
        <a class='bnt_duo' onclick=".'"'."del_img('"."#list_".$x."')".'"'." href='javascript:void(0)'><img src='".base_url()."views/skin/images/cross.png' /> 删除</a>
        </div>";
				  $x++;
