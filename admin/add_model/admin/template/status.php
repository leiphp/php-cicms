...
<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
 ...
 <?php }}else{?>
	...
	<?php } ?> 
     ...
     <div class="btn">
		<a class="button by"><i></i><input type="button" class="" value="通过审核" onclick="myform.action='<?=base_url();?>index.php/<$tablename>/status_ok?status=1';myform.submit();"/></a>
		<a class="button return"><i></i><input type="button" class="" value="退稿" onclick="myform.action='<?=base_url();?>index.php/<$tablename>/status_ok?status=2';myform.submit();"/></a>
