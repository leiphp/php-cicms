...
Util.Copy = function(pStr,hasReturn){
    ...
    else if (window.netscape)
    {
        ...
        {
            alert("您的firefox安全限制限制您进行剪帖板操作，请打开'about:config'将signed.applets.codebase_principal_support'设置为true'之后重试");
            return result;
        }
...
Util.selCategory = (function(){
    ...
    var start = function(obj){
        t.data = obj.data;
        t.parSel = document.getElementById(obj.parent);
        t.chiSel = document.getElementById(obj.chilren);
        t.chiSel.style.display = 'none';
        if(!obj.defSelect)obj.defSelect = ["请选择"," "];
        t.defTxt = obj.defSelect[0];
        t.defSign = obj.defSelect[1];
        getParent();
    ...
    var getParent = function(){
        ...
        t.parSel.options.add(new Option(t.defTxt,""));
        getPanOpt(); 
    }
    //获取子级
