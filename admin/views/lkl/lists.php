...
<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
    <tr>
		...
		<td>
		<a href="<?=base_url().'../lkl/show/'.$v['id'].'.html'?>" target="_blank"><?=$v['title']?></a>
		...
		<td align='center'><a href="<?=base_url();?>index.php/lkl/edit/<?=$v['id']?>">修改</a> | <a onclick="return ConfirmDel();"  href="<?=base_url();?>index.php/lkl/del/<?=$v['id']?>">删除</a></td>
	</tr>
 ...
 <?php }}else{?>
	...
	<?php } ?> 
     ...
     <div class="btn">
      <input type="button" class="button" value="百度url提交" onclick="myform.action='<?=base_url();?>index.php/common/baidu_url/lkl/';myform.submit();"/>
    	<input type="button" class="button" value="排序" onclick="myform.action='<?=base_url();?>index.php/lkl/order';myform.submit();"/>
