...
<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
    <tr>
		...
		<td>
		<a href="<?=base_url().'../anli/show/'.$v['id'].'.html'?>" target="_blank"><?=$v['title']?></a>
 ...
 <?php }}else{?>
	...
	<?php } ?> 
     ...
     <div class="btn">
		 <a class="button ulr"><i></i><input type="button" class="" value="百度url提交" onclick="myform.action='<?=base_url();?>index.php/common/baidu_url/anli/';myform.submit();"/></a>
		 <a class="button sorting"><i></i><input type="button" class="" value="排序" onclick="myform.action='<?=base_url();?>index.php/anli/order';myform.submit();"/></a>
