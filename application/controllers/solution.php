...
class Solution extends MY_Controller{
	public function __construct(){
		parent::__construct();
		$this->m=$this->load->model("Solutionm");//加载新闻模型
		$this->m=$this->Solutionm;//重命名新闻模型
	...
	public function lists(){
		...
		/*手机端响应式处理*/
		$w_style=$_GET['b'];
		$tp=auto_tp($tp,'wap/wap_solution_list',$w_style);
		/*手机端响应式结束*/
		if($tp=='wap/wap_solution_list'){$GLOBALS['wl_page']['solution']=6;}
		...
		//配置分页
		$GLOBALS['wl_page']['solution']=isset($GLOBALS['wl_page']['solution'])?$GLOBALS['wl_page']['solution']:10;
		$page_list = config_page($count,$this->model.'/lists/'.$pagecatid,$GLOBALS['wl_page']['solution']);
	...
	public function show(){
		...
		/*手机端响应式处理*/
		$w_style=$_GET['b'];
		$tp=auto_tp($tp,'wap/wap_solution_show',$w_style);
		/*手机端响应式结束*/
