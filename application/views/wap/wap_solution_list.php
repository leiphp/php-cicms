	...
	<section class="page_box">
	  <div class="yd_wap_news3 w_wap_news2">
		...
		<?php if(!empty($data['list'])){ foreach ($data['list'] as $v){?>
		  <li><a href="<?=$category['tablename']?>/show/<?=$v['id']?>.html" target="_self"><img src="<?=$v['image']?>" alt="" class="yl_auto" style="width: 145.5px; height: 111.923px;"><span><?=$v['title']?></span></a></li>
		<?php }}else{?> 
